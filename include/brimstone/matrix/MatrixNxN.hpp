...
BS_ARRAY_DEFINE_GENERIC_METHODS( Matrix, T, data, BS_TMPL_2( typename T, std::size_t N ), BS_SPEC_3( T, N, N ) )
BS_ARRAY_DEFINE_METHODS( Matrix, T, data, BS_TMPL_2( typename T, std::size_t N ), BS_SPEC_3( T, N, N ) )
BS_MATRIX_DEFINE_METHODS( N, N, BS_TMPL_2( typename T, std::size_t N ), BS_SPEC_3( T, N, N ) )
BS_MATRIX_SQUARE_DEFINE_METHODS( N, BS_TMPL_2( typename T, std::size_t N ), BS_SPEC_3( T, N, N ) )
...
template< typename T, std::size_t N >
void Matrix< T, N, N >::setRow( const std::size_t row, const Vector< T, N >& values ) {
...
template< typename T, std::size_t N >
void Matrix< T, N, N >::setColumn( const std::size_t col, const Vector< T, N >& values ) {
...
template< typename T, std::size_t N >
Vector< T, N > Matrix< T, N, N >::getRow( const std::size_t row ) const {
    BS_ASSERT_INDEX( row, N - 1 );
    Vector< T, N > out;
...
template< typename T, std::size_t N >
Vector< T, N > Matrix< T, N, N >::getColumn( const std::size_t col ) const {
    BS_ASSERT_INDEX( col, N - 1 );
    Vector< T, N > out;
    ...
    for( std::size_t r = 0; r < N; ++r ) {
        for( std::size_t c = 0; c < N; ++c ) {
            ...
            for( std::size_t r2 = 0; r2 < r; ++r2 ) {
                ...
                for( std::size_t c2 = c+1; c2 < N; ++c2 )
                    minorMatrix.elem[r2][c2-1] = matrix.elem[r2][c2];
            }
            for( std::size_t r2 = r+1; r2 < N; ++r2 ) {
                for( std::size_t c2 = 0;   c2 < c; ++c2 )
