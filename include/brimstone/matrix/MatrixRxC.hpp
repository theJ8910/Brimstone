...
#define BS_MATRIX_DECLARE_METHODS( R, C )                                             \
    ...
    bool isIdentity() const;                                                          \
                                                                                      \
    void setRow( const std::size_t row, const Vector< T, C >& values );               \
    void setColumn( const std::size_t col, const Vector< T, R >& values );            \
                                                                                      \
    Vector< T, C > getRow( const std::size_t row ) const;                             \
    Vector< T, R > getColumn( const std::size_t col ) const;                          \
                                                                                      \
    template< typename T2 >                                                           \
...
#define BS_MATRIX_DEFINE_METHODS( R, C, tmpl, spec )                                  \
    tmpl                                                                              \
    std::size_t Matrix spec::getRows() const {                                        \
        return R;                                                                     \
    }                                                                                 \
    tmpl                                                                              \
    std::size_t Matrix spec::getColumns() const {                                     \
        return C;                                                                     \
    }                                                                                 \
    tmpl                                                                              \
    ...
    tmpl                                                                              \
    T& Matrix spec::operator()( const std::size_t row, const std::size_t col ) {      \
        BS_ASSERT_INDEX( row, R - 1 );                                                \
        BS_ASSERT_INDEX( col, C - 1 );                                                \
        return elem[row][col];                                                        \
    }                                                                                 \
    tmpl                                                                              \
    T Matrix spec::operator()( const std::size_t row, const std::size_t col ) const { \
        BS_ASSERT_INDEX( row, R - 1 );                                                \
        BS_ASSERT_INDEX( col, C - 1 );                                                \
        return elem[row][col];                                                        \
    }
...
BS_ARRAY_DEFINE_GENERIC_METHODS( Matrix, T, data, BS_TMPL_3( typename T, std::size_t R, std::size_t C ), BS_SPEC_3( T, R, C ) )
BS_ARRAY_DEFINE_METHODS( Matrix, T, data, BS_TMPL_3( typename T, std::size_t R, std::size_t C ), BS_SPEC_3( T, R, C ) )
BS_MATRIX_DEFINE_METHODS( R, C, BS_TMPL_3( typename T, std::size_t R, std::size_t C ), BS_SPEC_3( T, R, C ) )
...
template< typename T, std::size_t R, std::size_t C >
void Matrix< T, R, C >::setRow( const std::size_t row, const Vector< T, C >& values ) {
    BS_ASSERT_INDEX( row, R - 1 );
    for( std::size_t c = 0; c < C; ++c )
...
template< typename T, std::size_t R, std::size_t C >
void Matrix< T, R, C >::setColumn( const std::size_t col, const Vector< T, R >& values ) {
    BS_ASSERT_INDEX( col, C - 1 );
    for( std::size_t r = 0; r < R; ++r )
...
template< typename T, std::size_t R, std::size_t C >
Vector< T, C > Matrix< T, R, C >::getRow( const std::size_t row ) const {
    BS_ASSERT_INDEX( row, R - 1 );
    Vector< T, C > out;
...
template< typename T, std::size_t R, std::size_t C >
Vector< T, R > Matrix< T, R, C >::getColumn( const std::size_t col ) const {
    BS_ASSERT_INDEX( col, C - 1 );
    Vector< T, R > out;
