...
WindowsWindow::WindowsWindow( Window& cParent ) : m_cParent( cParent ) {
...
bool WindowsWindow::processEvents() {
    ...
    while( PeekMessage( &m_msg, NULL, 0, 0, PM_REMOVE ) == TRUE )
	{
...
void WindowsWindow::adjustWindowBounds( long iWidth, long iHeight ) {
	...
	//MoveWindow is sort of a misnomer. You're actually framing the window in a new rectange, meaning you can position and/or resize.
...
LRESULT CALLBACK WindowsWindow::mainProc( HWND hWnd, UINT uiMessage, WPARAM wParam, LPARAM lParam ) {
	...
	case WM_MOUSEMOVE: {
        LongRectangle cRect;
        pcWindow->m_cParent.getBounds( cRect );
        ...
        pcWindow->m_cParent.m_cSignalMouseMove( cEvent );  
    } break;
    case WM_LBUTTONDOWN: {
        MousePressEvent cEvent( MouseButton::LEFT );
        pcWindow->m_cParent.m_cSignalMousePress( cEvent );
    } break;
    case WM_LBUTTONUP: {
        MouseReleaseEvent cEvent( MouseButton::LEFT );
        pcWindow->m_cParent.m_cSignalMouseRelease( cEvent );
    } break;
    case WM_RBUTTONDOWN: {
